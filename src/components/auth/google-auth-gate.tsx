...
import { Layers, ShieldCheck, ArrowRight, User } from 'lucide-react';
...
export function GoogleAuthGate() {
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      toast.success('Signed in with Google as Alex Morgan');
    }, 600);
  };
  ...
  const handleCustomSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      toast.success(`Signed in as ${customName}`);
    }, 600);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950 p-4 text-zinc-100 selection:bg-zinc-800">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-900/90 p-8 shadow-2xl backdrop-blur-xl z-10"
      >
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h2 className="text-xl font-bold tracking-tight text-white">HariSumiran Tasks</h2>
          <p className="text-xs text-zinc-400 mt-1 max-w-[260px]">
            Minimal, high-agency project workspace. Please authenticate to continue.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center gap-3 rounded-2xl border border-zinc-700 bg-zinc-800/80 px-4 py-3.5 text-sm font-semibold text-white shadow-md hover:bg-zinc-800 hover:border-zinc-600 active:scale-[0.98] transition-all cursor-pointer disabled:opacity-50"
          >
            {/* Google SVG Icon */}
          ...
          <div className="pt-2 text-center">
            ...
            >
              {showCustom ? 'Hide custom input' : 'Use a different Google email address'}
            </button>
          </div>
          ...
          {showCustom && (
            <form onSubmit={handleCustomSubmit} className="space-y-2 pt-2 animate-in fade-in-50">
              <input
                ...
                className="w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2 text-xs text-white placeholder:text-zinc-600 focus:outline-hidden focus:border-zinc-500"
              />
              <input
                type="email"
                value={customEmail}
                onChange={(e) => setCustomEmail(e.target.value)}
                placeholder="Google Email (e.g. maya@gmail.com)"
                className="w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2 text-xs text-white placeholder:text-zinc-600 focus:outline-hidden focus:border-zinc-500"
              />
              <button
                type="submit"
        ...
        <div className="mt-8 flex items-center justify-center gap-1.5 text-[11px] text-zinc-500 border-t border-zinc-800/80 pt-4">
          <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" />
          <span>OAuth 2.0 Verified & Persistent Backend Connected</span>
        </div>
      </motion.div>
    </div>
  );
